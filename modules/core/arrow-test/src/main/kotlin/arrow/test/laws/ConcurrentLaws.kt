  ...
  fun <F> Concurrent<F>.asyncCanCancelUpstream(EQ: Eq<Kind<F, Int>>, ctx: CoroutineContext) =
    forAll(Gen.int()) { i ->
      bindingCancellable {
        ...
        upstream.followedBy(downstream).fork(ctx).bind()
  ...
  fun <F> Concurrent<F>.raceCanBeCancelledByParticipants(EQ: Eq<Kind<F, Int>>, ctx: CoroutineContext) =
    forAll(Gen.int(), Gen.bool()) { i, shouldLeftCancel ->
      bindingCancellable {
        ...
        if (shouldLeftCancel) ctx.raceN(cancel, loser).fork(ctx).bind()
        else ctx.raceN(loser, cancel).fork(ctx).bind()
  ...
  fun <F> Concurrent<F>.racePairCanBeCancelledByParticipants(EQ: Eq<Kind<F, Int>>, ctx: CoroutineContext) =
    forAll(Gen.int(), Gen.bool()) { i, shouldLeftCancel ->
      bindingCancellable {
        ...
        if (shouldLeftCancel) ctx.racePair(cancel, loser).fork(ctx).bind()
        else ctx.racePair(loser, cancel).fork(ctx).bind()
  ...
  fun <F> Concurrent<F>.raceTripleCanBeCancelledByParticipants(EQ: Eq<Kind<F, Int>>, ctx: CoroutineContext) =
    forAll(Gen.int(), Gen.from(listOf(1, 2, 3))) { i, shouldCancel ->
      bindingCancellable {
        ...
        when (shouldCancel) {
          1 -> ctx.raceTriple(cancel, loser, loser2).fork(ctx).bind()
          2 -> ctx.raceTriple(loser, cancel, loser2).fork(ctx).bind()
          else -> ctx.raceTriple(loser, loser2, cancel).fork(ctx).bind()
        }
  ...
  fun <F> Concurrent<F>.parMapCanBeCancelledByParticipants(EQ: Eq<Kind<F, Int>>, ctx: CoroutineContext) =
    forAll(Gen.int(), Gen.bool()) { i, shouldLeftCancel ->
      bindingCancellable {
        ...
        if (shouldLeftCancel) ctx.parMapN(cancel, loser, ::Tuple2).fork(ctx).bind()
        else ctx.parMapN(loser, cancel, ::Tuple2).fork(ctx).bind()
