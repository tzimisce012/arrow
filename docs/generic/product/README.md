...
// HCons(head=1000, tail=HCons(head=900, tail=arrow.generic.HNil@66ebdb64))
```

```kotlin
Account(1000, 900).toHListLabeled()
// HCons(head=Tuple2(a=balance, b=1000), tail=HCons(head=Tuple2(a=available, b=900), tail=arrow.generic.HNil@66ebdb64))
```
...
// DeferredK(deferred=LazyDeferredCoroutine{New}@64ecb88f, scope=kotlinx.coroutines.GlobalScope@446c5565)
```
